...
class PhaseManager {
  ...
  moveToNextPhase() {
    ...
    if (currentPhaseId === 'vote') {
      ...
      if (context && context.data && context.data.needsRunoff === true) {
        // 同票の場合は決選投票フェーズに移行
        const runoffCandidates = context.data.maxVoted || [];
        this.moveToPhase('runoffVote');
        // 決選投票用のコンテキストデータを設定
        this.updatePhaseContextData({
          runoffCandidates
        });
        return this.currentPhase;
      }
    ...
    const applicableRules = this.phaseTransitions.filter(rule => {
      ...
      const sourceMatches = rule.sourcePhase === currentPhaseId || rule.sourcePhase === '*';
      
      // 条件が満たされている
      const conditionMet = rule.condition(this.game);
      
      return sourceMatches && conditionMet;
    });
  ...
  finalizeCurrentPhase() {
    ...
    if (this.currentPhaseContext) {
      ...
      // 履歴に追加（新しいオブジェクトを作成して追加）
      const finalizedContext = {
        phaseId: this.currentPhase.id,
        turn: this.currentTurn,
        startTime: this.currentPhaseContext.startTime,
        endTime: this.currentPhaseContext.endTime,
        status: 'completed',
        data: { ...this.currentPhaseContext.data }
      };
